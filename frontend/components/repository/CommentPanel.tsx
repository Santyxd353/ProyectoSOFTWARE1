...
export default function CommentPanel({ workspaceId, revisionId, fileId, canComment }: CommentPanelProps) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!trimmed || trimmed.length > 4000) {
      setError('El comentario debe tener entre 1 y 4000 caracteres.');
      return;
    }
    setLoading(true);
    setError('');
    try {
      ...
      setError(requestError.response?.data?.message || 'No se pudo guardar el comentario.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <aside className="rounded-lg border border-border bg-card p-4" aria-labelledby="comments-title">
      <h3 id="comments-title" className="mb-3 font-semibold text-card-foreground">Comentarios</h3>
      <div className="max-h-64 space-y-3 overflow-auto" aria-live="polite">
        {comments.length === 0 && <p className="text-sm text-muted-foreground">Todavía no hay observaciones en este archivo.</p>}
        {comments.map((comment) => (
          <article key={comment.id} className="rounded-md bg-muted p-3 text-sm">
            <div className="flex items-center justify-between gap-2">
              ...
              {comment.line && <span className="text-xs text-muted-foreground">Línea {comment.line}</span>}
            </div>
      ...
      {canComment ? (
        ...
        <p className="mt-4 text-sm text-muted-foreground">Tu rol permite lectura, pero no agregar comentarios.</p>
      )}
    </aside>
  );
}
