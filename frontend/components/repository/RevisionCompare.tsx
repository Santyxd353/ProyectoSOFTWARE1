...
import { RevisionComparison, RevisionSummary } from '@/types/repository';
...
export default function RevisionCompare({ workspaceId, revisions }: { workspaceId: string; revisions: RevisionSummary[] }) {
  ...
  const compare = async () => {
    ...
    try {
      ...
      setError(requestError.response?.data?.message || 'No se pudieron comparar las revisiones.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section className="rounded-lg border border-border bg-card p-4" aria-labelledby="compare-title">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h3 id="compare-title" className="font-semibold text-card-foreground">Comparar revisiones</h3>
      </div>
          ...
          {result.files.map((file) => (
            <article key={file.path} className="overflow-hidden rounded-md border border-border">
              <header className="flex flex-wrap items-center justify-between gap-2 bg-muted px-3 py-2">
                ...
                <span className="rounded bg-background px-2 py-1 text-xs font-medium text-muted-foreground">{file.kind.replace('_', ' ')}</span>
              </header>
